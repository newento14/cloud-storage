...
const Table = () => {
    ...
    function nameClickHandler() {
        ...
        setSortParams({name: value, size: 0, date: 0, selected: 0});
    }

    function sizeClickHandler() {
        const value = sortParams.size === 0 ? 1 : sortParams.size === 2 ? 1 : 2;
        setSortParams({name: 0, size: value, date: 0, selected: 0});
    }

    function dateClickHandler() {
        const value = sortParams.date === 0 ? 1 : sortParams.date === 2 ? 1 : 2;
        setSortParams({name: 0, size: 0, date: value, selected: 0});
    }

    function selectedClickHandler() {
        const value = sortParams.selected === 0 ? 1 : sortParams.selected === 2 ? 1 : 2;
        setSortParams({name: 0, size: 0, date: 0, selected: value});
    }
    ...
    const sortedArray = useMemo(() => {
        ...
        res.sort((a, b) => {
            ...
            if (sortParams.selected === 1) {
                return a.starred ? -1 : 1;
            }
            if (sortParams.selected === 2) {
    ...
    return (
        <div>
            <div className="table-header">
                ...
                <div className="table-selected" onClick={selectedClickHandler}>
                    Selected
                    {sortParams.selected !== 0 && (
                        <img
                            src={sortParams.selected === 1 ? ArrowUp : ArrowDown}
                            className="icon"
                            alt="sortArrow"
                        />
                    )}
